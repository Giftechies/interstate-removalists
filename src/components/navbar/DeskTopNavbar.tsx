...
const DeskTopNavbar = ({ scrollHight }: { scrollHight: number }) => {
  const pathName = usePathname();

  return (
    <div
      className={cn("theme-transition-3 fixed top-0 z-50 w-full", {
        "theme-transition-3 bg-white-1 shadow-sm": scrollHight > 50,
      })}
    >
      <div className="transition-primary spy24px container theme-transition-3 hidden w-full items-center justify-between xl:flex">
          ...
          {navbarData.map(({ id, menuTitle, path, menuItems }) => {
            ...
            return menuItems ? (
              <li key={`${id}`} className="group relative">
                <p
                  className={cn(
                    "theme-transition-3 relative z-10 flex cursor-pointer items-center justify-between gap-1 text-black-4 before:theme-transition-3 before:absolute before:bottom-0  before:block before:h-px before:w-0 before:bg-prim hover:text-prim hover:before:w-full",
        ...
        <div className="flex gap-8">
          <Link
            href={"/login"}
            className="l-text theme-transition-3 flex items-center gap-1 text-black-4 hover:text-prim "
          >
            <IconLock />
            <span>Login</span>
          </Link>
