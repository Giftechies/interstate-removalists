...
const LInkArrow = ({
  ...
  return (
    <Link
      href={`${link}`}
      className={cn(
        "group flex items-center gap-16 hover:text-[var(--secondary-hex)]",
        className,
      )}
    >
      <span>{linkText}</span>
      ...
      <span className="relative flex items-center">
        ...
        >
          <path
            ...
            className="theme-transition-3 fill-black-1 group-hover:fill-[var(--secondary-hex)]"
          />
        </svg>
        <span
          className={cn(
            "block size-12 rounded-full border border-black-1 group-hover:border-[var(--secondary-hex)]",
            roundClassName,
          )}
        ></span>
      </span>
